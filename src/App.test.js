...
describe("<App />", () => {
  it("should fetch data and display it to the screen", async () => {
    mockFetchShow.mockResolvedValue({ data });

    const { getByText } = render(<App />);

    const loadingText = getByText(/fetching data.../i);
  ...
  it("should change dropDown value when clicked", () => {
    const onChange = jest.fn();
    render(<Dropdown onChange={onChange} />, <App />);

    const dropDwn = screen.getByText(/select a season/i).toBeInTheDocument();

    dropDwn.fireEvent.onChange(getByText(/season 1/i));
